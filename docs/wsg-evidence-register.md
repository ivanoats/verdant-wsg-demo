...
- **R1** `README.md:1-30`
- **R2** `package.json:1-16`
- **R3** `package-lock.json:1-14`
- **R4** `netlify.toml:1-6`
- **R5** `public/_headers:1-22`
- **R6** `panda.config.ts:1-140`
- **R7** `scripts/build.mjs:25-249`
- **R8** `scripts/build.mjs:327-391`
- **R9** `scripts/stats.mjs:1-46`
- **R10** `scripts/fingerprint.mjs:1-42`
- **R11** Repository tree review on 2026-09-22: no `LICENSE`, `CONTRIBUTING.md`, `.github/`, or Git tags were present in the checked-out repository.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| UX | Accessible, low-bandwidth implementation patterns | The repo contains implementation evidence for skip links, landmarks, labelled form fields, system fonts, dark mode via `prefers-color-scheme`, motion gating, and decorative-image reduction under reduced-data preferences. | R1, R6, R7, R8 | Repository implementation | 2026-09-22 | Verified | Keep this register updated if the site architecture changes. |
...
| Development | Static build architecture and shipped-weight evidence | The site is generated from `scripts/build.mjs`, styled via PandaCSS static extraction, and measured at build time with `scripts/stats.mjs`; Netlify build config pins `npm run build` on Node 20. | R1, R4, R6, R7, R9 | Repository build process | 2026-09-22 | Verified | None beyond normal maintenance. |
...
1. **What is currently verifiable**
   - The published site uses a `netlify.app` hostname, which Netlify documents as its default provider-managed subdomain for deployed sites. `netlify.toml` and `public/_headers` also match documented Netlify deployment features. This is enough to identify the hosting platform, but not its electricity source. (R4, R5, P2, P3, P4)
